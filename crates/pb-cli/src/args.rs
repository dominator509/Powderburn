...
pub struct Args {
    ...
    /// Output path for save (--output or -o).
    pub output: Option<PathBuf>,
    /// Input path for load/resume (--input or -i).
    pub input: Option<PathBuf>,
    /// Path to a campaign save file.
    ...
    pub fn parse(raw: Vec<OsString>) -> Result<Self, String> {
        ...
        while i < raw.len() {
            ...
            match arg.as_str() {
                ...
                "--output" | "-o" => {
                    i += 1;
                    let val = raw.get(i).ok_or("--output requires a value")?;
                    output = Some(PathBuf::from(val));
                }
                "--input" | "-i" => {
                    i += 1;
                    let val = raw.get(i).ok_or("--input requires a value")?;
                    input = Some(PathBuf::from(val));
                }
                "--campaign" => {
                    i += 1;
                    let val = raw.get(i).ok_or("--campaign requires a value")?;
                    campaign_path = Some(PathBuf::from(val));
                }
                "--seed" => {
                    i += 1;
